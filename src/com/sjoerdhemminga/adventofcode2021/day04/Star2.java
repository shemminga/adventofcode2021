...
public final class Star2 {
    ...
    private static void playBingo(final Deque<Integer> numbers, final List<int[][]> boards) {
        ...
        OptionalInt losingBoard = OptionalInt.empty();
        int drawn = -1;
        while (losingBoard.isEmpty()) {
            drawn = numbers.pop();
            markBoards(drawn, marked, boards);
            losingBoard = getLosingBoard(marked);
        }

        final int loserIdx = losingBoard.getAsInt();

        while (!isWinningBoard(marked.get(loserIdx))) {
            drawn = numbers.pop();
            markBoard(drawn, marked.get(loserIdx), boards.get(loserIdx));
        }

        printLoser(drawn, marked.get(loserIdx), boards.get(loserIdx));
    }

    private static void printLoser(final int lastDrawn, final boolean[][] marked, final int[][] board) {
    ...
    private static OptionalInt getLosingBoard(final List<boolean[][]> marked) {
        final List<boolean[][]> stillPlaying = IntStream.range(0, marked.size())
                .filter(i -> !isWinningBoard(marked.get(i)))
                .mapToObj(marked::get)
                .toList();

        if (stillPlaying.size() != 1) return OptionalInt.empty();

        return OptionalInt.of(marked.indexOf(stillPlaying.get(0)));
    }
