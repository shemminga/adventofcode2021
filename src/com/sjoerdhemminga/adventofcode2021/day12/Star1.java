...
public final class Star1 {
    ...
    private static int countRoutesFromTo(final boolean[][] graph, final String[] vertices, final boolean[] isLarge,
        ...
        route(graph, vertices, isLarge, new boolean[vertices.length], new ArrayDeque<>(), routeCounter, start,
                end);
    ...
    private static void route(final boolean[][] graph, final String[] vertices, final boolean[] isLarge,
            final boolean[] visited, final Deque<Integer> route, final AtomicInteger routeCounter, final int current,
            final int end) {
        if (visited[current] && !isLarge[current]) return;
        ...
        if (current == end) {
            ...
            for (int i = 0; i < graph[current].length; i++)
                if (graph[current][i])
                    route(graph, vertices, isLarge, visited, route, routeCounter, i, end);
